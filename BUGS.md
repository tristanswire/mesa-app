...
- **Cook Mode swipe + M/L/XL text size (Round 3a, animated in Round 3a-anim)** — swipe-to-navigate uses **`react-native-gesture-handler` + `react-native-reanimated@~4.1.1`**, and the step transition is an **animated slide** (280ms, `Easing.out(Easing.cubic)`), interactive during the drag. Step content is a 3-pane carousel (prev/current/next, absolutely positioned at ∓1 screen width) in `CookStepPane.tsx`, translated by a single shared value in `CookModeView.tsx`.
  - **Babel: `react-native-worklets/plugin` MUST stay LAST in `babel.config.js` plugins.** Reanimated 4 moved the plugin out of `react-native-reanimated/plugin` (that was v3). It rewrites worklet bodies and must see every other transform's output — a plugin added after it breaks animations at runtime with **no build error**.
  - **`react-native-worklets@0.8.0` is a transitive native dependency** — required peer of Reanimated 4, present in `node_modules` but **not listed in `package.json`**. `expo install --check` accepts this. If autolinking ever misses it, add it explicitly rather than downgrading Reanimated.
  - **First local run after pulling this change needs `npx expo start -c`** (stale Metro cache + new Babel plugin = confusing runtime failures).
  - Fragile spots: horizontal-swipe vs. vertical-scroll inside each pane's `ScrollView` (disambiguated by `activeOffsetX`/`failOffsetY`; if they fight, swap to RNGH's `ScrollView` or add `simultaneousWithExternalGesture`); timer-chip taps being eaten by the pan; a one-frame flash at the index swap if the `translateX` reset ever moves out of the `useLayoutEffect` keyed on `stepIndex`; an over-swipe on the last step must NOT finish the cook (guarded — finishing stays the button). Three ScrollViews now mount per step instead of one — watch performance on older devices.
  - Slider scales step text only, caps Dynamic-Type compounding at 1.4×, and persists via AsyncStorage (`mesa.cookmode.textSize`). Needs a native dev build to run (gesture-handler **and** Reanimated are native modules) — Metro reload won't exercise it.
